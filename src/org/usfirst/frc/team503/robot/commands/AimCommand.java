...
public class AimCommand extends Command {
	...
	protected void execute() {
		...
		// if(NavSensorSubsystem.ahrs.getFusedHeading()> ((degrees +tolerance+ 180)
		// %360)){
		// DrivetrainSubsystem.instance.tankDrive(RobotMap.AUTON_DRIVE_SPEED,
		// -RobotMap.AUTON_DRIVE_SPEED, RobotMap.SENSITIVITY);
		// }
		...
		if (NavSensorSubsystem.ahrs.getYaw() < degrees - tolerance) {
			...
			if (NavSensorSubsystem.ahrs.getYaw() < degrees - midTolerance) {
				NewDrivetrainSubsystem.instance.tankDrive(-RobotMap.AUTON_MID_DRIVE_SPEED,
						RobotMap.AUTON_MID_DRIVE_SPEED, false);
	...
	protected boolean isFinished() {
		return ((NavSensorSubsystem.ahrs.getYaw() <= (degrees + lowTolerance))
				&& (NavSensorSubsystem.ahrs.getYaw() >= (degrees - lowTolerance)));
	}
