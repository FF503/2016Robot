...
public class AnilSTurn extends Command {
	...
	protected void execute() {
		...
		/*
		 * else if(step<4){ NewDrivetrainSubsystem.instance.tankDrive(-.6, -.6, false);
		 * if(NewDrivetrainSubsystem.instance.getEncoderDistance()> toBatter){ step = 4;
		 * time.start(); } } else if(step<5){
		 * ArmSubsystem.instance.setArmWinchMotor(RobotMap.ARM_SPEED); if(time.get()>2
		 * || ArmSubsystem.instance.getArmLowerLimitSwitch()){ step = 5;
		 * NewDrivetrainSubsystem.instance.resetEncoder(); } } else if(step<6){
		 * NewDrivetrainSubsystem.instance.tankDrive(-.6, -.6, false);
