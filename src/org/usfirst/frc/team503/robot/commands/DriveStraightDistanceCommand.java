...
public class DriveStraightDistanceCommand extends Command {
	...
	protected void initialize() {
		time = new Timer();
		time.start();
		NewDrivetrainSubsystem.driveEncoder.reset();
		NewDrivetrainSubsystem.instance.setSetpoint(inches);
		NewDrivetrainSubsystem.instance.enable();
		SmartDashboard.putNumber("Setpoint=", inches);
		NavSensorSubsystem.ahrs.reset();

		initAngle = 0;
		// NewDrivetrainSubsystem.instance.tankDrive(-RobotMap.AUTON_DRIVE_SPEED,
		// -RobotMap.AUTON_DRIVE_SPEED, false);
	}
	...
	protected void execute() {
		 ...
		 * SmartDashboard.putNumber("encoder kounts",
		 * NewDrivetrainSubsystem.instance.getEncoderCount());
		 * SmartDashboard.putNumber("HOW FAR DID IT GO??????",
		 * NewDrivetrainSubsystem.instance.getEncoderDistance());
		...
		if (NavSensorSubsystem.ahrs.getYaw() > (initAngle + RobotMap.COMPASS_TOLERANCE)) {
			NewDrivetrainSubsystem.instance.tankDrive((-RobotMap.drivePIDOutput * 3.0 / 4), -RobotMap.drivePIDOutput,
					false);
		} else if (NavSensorSubsystem.ahrs.getYaw() < (initAngle - RobotMap.COMPASS_TOLERANCE)) {
			NewDrivetrainSubsystem.instance.tankDrive(-RobotMap.drivePIDOutput, (-RobotMap.drivePIDOutput * 3 / 4),
					false);
		} else {
			NewDrivetrainSubsystem.instance.tankDrive(-RobotMap.drivePIDOutput, -RobotMap.drivePIDOutput, false);
		}
		// NewDrivetrainSubsystem.instance.tankDrive(-RobotMap.drivePIDOutput,
		// -RobotMap.drivePIDOutput, false);
	...
	protected boolean isFinished() {
		...
		// if actual distance traveled on encoders > than set point we have reached
		// destination
		// if (NewDrivetrainSubsystem.driveEncoder.getDistance() >=
		// NewDrivetrainSubsystem.instance.getSetpoint()-1.0)
