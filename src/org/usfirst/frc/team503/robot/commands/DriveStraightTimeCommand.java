...
public class DriveStraightTimeCommand extends Command {
	...
	protected void execute() {
		...
		} else if (NavSensorSubsystem.ahrs.getFusedHeading() < (((initAngle - RobotMap.COMPASS_TOLERANCE) < 0)
				? (initAngle - RobotMap.COMPASS_TOLERANCE) % 360 + 360
				: initAngle - RobotMap.COMPASS_TOLERANCE)) {
		 ...
		 * } else if(NavSensorSubsystem.ahrs.getFusedHeading()<(initAngle-RobotMap.
		 * COMPASS_TOLERANCE)){
		 * DrivetrainSubsystem.instance.tankDrive(-RobotMap.AUTON_DRIVE_SPEED,0, false);
		 * } else{ DrivetrainSubsystem.instance.tankDrive(-RobotMap.AUTON_DRIVE_SPEED,
