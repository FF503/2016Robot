...
public class VisionProcessor extends Subsystem {
	...
	public void refresh() {
		...
		try {
			...
			/*
			 * centerXs[0] = 160; //point 1 centerYs[0] = 120;
			 * 
			 * centerXs[1] = 160; //point 2 centerYs[1] = 120;
			 * 
			 * centerXs[2] = 160; //point 3 centerYs[2] = 120;
			 * 
			 * centerXs[3] = 160; //point 4 centerYs[3] = 120;
			 */
	...
	// return Math.abs((getCenterX() - IMAGE_CENTER_X)) * PIXEL_RATIO <
	// RobotMap.COMPASS_TOLERANCE;
	// }
	...
	public double approximateDistance() {
		...
		// double distance = ((FOCAL_LENGTH * GOAL_HEIGHT *
		// IMAGE_HEIGHT)/(myHeight(foundGoal)*CAMERA_HEIGHT)/25.4);// distance in inches
	...
	public double getAngle() {
		...
		/*
		 * while(count < 101){ totalDegrees+=NavSensorSubsystem.ahrs.getYaw() +
		 * PIXEL_RATIO * (centerX - VisionProcessor.IMAGE_CENTER_X); count ++; }
		 */
		...
		if (targetFound() && getCameraDistance() < 8) {
			...
			/*
			 * if(centerX > 120){ degrees = (PIXEL_RATIO * (centerX -
			 * VisionProcessor.IMAGE_CENTER_X ))+10; }
			 */
		...
		// if(Math.abs((PIXEL_RATIO * (centerX - VisionProcessor.IMAGE_CENTER_X))) >=
		// RobotMap.COMPASS_TOLERANCE)
		// degrees = NavSensorSubsystem.ahrs.getYaw() + PIXEL_RATIO * (centerX -
		// VisionProcessor.IMAGE_CENTER_X);
		// else{
		// degrees = 0;
		// }
		// if (degrees < 0){
	...
	// if(Math.abs((PIXEL_RATIO * (centerX - VisionProcessor.IMAGE_CENTER_X))) >=
	// RobotMap.COMPASS_TOLERANCE)
	// degrees = NavSensorSubsystem.ahrs.getFusedHeading() + PIXEL_RATIO * (centerX
	// - VisionProcessor.IMAGE_CENTER_X);
	// else{
