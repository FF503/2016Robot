...
public class ArmSubsystem extends PIDSubsystem {
	...
	private ArmSubsystem() {
		...
		armAngleEncoder = new Encoder(RobotMap.armEncoderPortA, RobotMap.armEncoderPortB,
				RobotMap.armEncoderReverseDirection, RobotMap.encoderType);
		...
		// extenderEncoder = new Encoder(RobotMap.extenderEncoderPortA,
		// RobotMap.extenderEncoderPortB, RobotMap.extenderEncoderReverseDirection,
		// RobotMap.encoderType);
	...
	public static enum ArmPosition {
		TOP(RobotMap.ARM_POSITION_TOP), LOAD(RobotMap.ARM_POSITION_LOAD), BOT(RobotMap.ARM_POSITION_BOT), OPERATOR(
				7000), PBOTTOP(RobotMap.ARM_POSITION_PBOT_TOP);
	...
	public boolean getArmUpperLimitSwitch() {
		// SmartDashboard.putBoolean("arm upper limit switch",
		// armUpperLimitSwitch.get());
		return false;
	}
	...
	public double getArmAngleEncoderError(ArmPosition position) {
		// SmartDashboard.putNumber("arm encoder error", Math.abs(armAngleEncoder.get()
		// - position.position));
	...
	protected void usePIDOutput(double output) {
		...
		/*
		 * else if(output >-.08&& output <0){ output = -.08; }
		 */
